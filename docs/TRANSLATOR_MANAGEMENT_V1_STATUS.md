...
### 3.4 譯者語言能力（UI 暫緩，DB 保留）

以下欄位**資料表已存在**，但新增／編輯譯者表單**暫不提供 UI**（2026-06 起）：

| 欄位 | 表 | 說明 |
|------|-----|------|
| `native_lang` | `translator_master` | 母語／主要語系代碼（NOT NULL）；新增時後端寫入佔位值 `und` |
| `language_skills` | `translator_master` | 語言能力 JSON 陣列；新增時預設 `[]`，編輯時保留 DB 既有值 |

**下一版待建置**：

- 表單：母語下拉（對齊站內語系代碼）與語言能力多選
- 列表：可選擇性恢復「語言別」欄位顯示
- 派案：與 `get_dispatch_translators`、語種費率規則整合
