...
export default async function FinancePage({params}: {params: Promise<{locale: string}>}) {
  ...
  const financeT = await getTranslations({locale, namespace: "Finance"});
  ...
  return (
    ...
    >
      <div className="space-y-2">
        <h1 className="text-2xl font-semibold text-foreground">{financeT("title")}</h1>
        <p className="text-sm text-muted-foreground">{financeT("subtitle")}</p>
      </div>
    </DashboardShell>
  );
}
